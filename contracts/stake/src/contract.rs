...
use crate::{msg::{StakedResponse, AllStakedResponse, AnnualizedRewardsResponse, WithdrawableRewardsResponse, DistributedRewardsResponse}, error::ContractError};
...
pub trait StakingTrait {
    ...
    fn query_staked(
        env: Env,
        address: Address,
        ) -> Result<StakedResponse, ContractError>;

    fn query_all_staked(
        env: Env,
        ) -> Result<AllStakedResponse, ContractError>;

    fn query_annualized_rewards(
        env: Env,
        ) -> Result<AnnualizedRewardsResponse, ContractError>;
    ...
    fn query_distributed_rewards(
        env: Env,
        ) -> Result<DistributedRewardsResponse, ContractError>;
}
