...
pub fn compute_d(env: &Env, amp: u128, pools: &[Decimal256]) -> Decimal256 {
    let N_COINS = Decimal256::raw(&env, U256::from_u128(&env, 2000000000000000000));
    let leverage = Decimal256::from_ratio(&env, amp, U256::from_u128(&env, AMP_PRECISION as u128)).mul(&env, N_COINS);
    ...
    for _ in 0..ITERATIONS {
        let d_product = d.pow(3).div(&(amount_a_times_coins * amount_b_times_coins));
        d_previous = d.clone();
...
fn calculate_step(
    ...
    let leverage_sub = initial_d.mul(&leverage.sub(&Decimal256::one()));

    // (n_coins + 1) * d_product
    let n_coins_sum = d_product.mul(&(N_COINS + Decimal256::one()));
...
pub(crate) fn calc_y(
    ...
    let N_COINS = Decimal256::raw(&env, U256::from_u128(&env, 2000000000000000000));
    let TOL = Decimal256::raw(&env, U256::from_u128(&env, 1000000000000));

    let d = compute_d(env, amp, xp);
    let leverage = Decimal256::from_ratio(&env, amp, U256::from_u128(&env, 1u128)) * N_COINS;
    let amp_prec = Decimal256::from_ratio(&env, U256::from_u128(&env, AMP_PRECISION), U256::from_u128(&env, 1u128));

    let c = d.pow(&env, 3).mul(&env, &amp_prec).div(
        &env,
        new_amount
    ...
    let b = new_amount.add(&d.mul(&env, &amp_prec).div(&env, &leverage));
    ...
    for _ in 0..ITERATIONS {
        y_prev = y.clone();
        y = y.pow(&env, 2) + c.div(&env, (y.mul(&env, &N_COINS) + b - d));
